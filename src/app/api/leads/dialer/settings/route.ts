...
export async function GET() {
  try {
    ...
    return NextResponse.json({ settings: toLeadDialerDto(settings), activeCalls });
  } catch (err) {
    return apiErrorResponse(err, "Failed to load lead dialer settings");
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    if (typeof body.filters === "object" && body.filters !== null) {
      ...
      const leadFilters = (incoming.leads ?? incoming) as LeadDialerFilters;
      data.filters = stringifyDialerFilters({
        leads: leadFilters,
      });
    }

    await getOrCreateLeadDialerSettings();
    const settings = await db.dialerSettings.update({
      where: { id: OpsSegment.LEADS },
      data,
    });
    return NextResponse.json({ settings: toLeadDialerDto(settings) });
  } catch (err) {
    ...
    return apiErrorResponse(err, "Failed to update lead dialer settings");
  }
}
