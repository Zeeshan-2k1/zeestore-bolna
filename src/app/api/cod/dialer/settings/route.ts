...
import { getOrCreateCodDialerSettings, toCodDialerDto } from "@/lib/cod-dialer";
import { db } from "@/lib/db";
import { CallStatus, OpsSegment } from "@/lib/constants";
import { stringifyDialerFilters } from "@/lib/dialer-filters";
import type { CodDialerFilters } from "@/lib/dialer-filters";
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ settings: toCodDialerDto(settings), activeCalls });
  } catch (err) {
    return apiErrorResponse(err, "Failed to load COD dialer settings");
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    if (typeof body.delayBetweenCallsMs === "number") {
      data.delayBetweenCallsMs = Math.min(60_000, Math.max(0, body.delayBetweenCallsMs));
    }
    if (typeof body.filters === "object" && body.filters !== null) {
      const incoming = body.filters as Record<string, unknown>;
      const codFilters = (incoming.cod ?? incoming) as CodDialerFilters;
      data.filters = stringifyDialerFilters({ cod: codFilters });
    }

    await getOrCreateCodDialerSettings();
    const settings = await db.dialerSettings.update({
      where: { id: OpsSegment.COD },
      data,
    });
    return NextResponse.json({ settings: toCodDialerDto(settings) });
  } catch (err) {
    ...
    return apiErrorResponse(err, "Failed to update COD dialer settings");
  }
}
